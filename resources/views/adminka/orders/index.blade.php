                        ...
                        <tbody>
                            <?php foreach($orders as $item): ?>
                                <tr>
                                    ...
                                    <td style="width:300px;"><a href="{{ route('adm-order-view', ['locale' => app()->getLocale(), 'order_id' => $item->id]) }}"><?=$item->title?></a></td>
                                    ...
                                    <td class="text-center"><a href="{{ route('adm-order-edit', ['locale' => app()->getLocale(), 'order_id' => $item->id]) }}">{{ trans('site.Edit') }}</a></td>
                                    <td class="text-center"><a href="{{ route('adm-order-delete', ['locale' => app()->getLocale(), 'order_id' => $item->id]) }}" onclick="return confirm('Удалить этот заказ?');">{{ trans('site.Delete') }}</a></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
